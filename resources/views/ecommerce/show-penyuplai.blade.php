...
@section('title')
    <title>Jual {{ $penyuplai->name }}</title>
@endsection
		...
		<div class="banner_inner d-flex align-items-center">
			<div class="container">
				<div class="banner_content text-center">
                    <h2>{{ $penyuplai->name }}</h2>
					<div class="page_link">
                        <a href="{{ url('/') }}">Home</a>
                        <a href="#">{{ $penyuplai->name }}</a>
					</div>
				</div>
			</div>
		</div>
					...
					<div class="s_product_img">
						<div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
							<div class="carousel-inner">
								<h3 class="text-center">{{ $penyuplai->name }}</h3>
								<div class="carousel-item active">
									<img class="d-block w-100" src="{{ asset('storage/products/' . $penyuplai->image) }}" alt="{{ $penyuplai->name }}">
								</div>
							</div>
						</div>
					</div>
