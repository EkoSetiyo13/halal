                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <tr>
                        ...
                        <td>
                          <h5>: {{ $binaan->no_wa }} <a class="btn btn-success btn-sm" target="_blank"
                                                        href="https://api.whatsapp.com/send?phone=62{{ substr($binaan->no_wa, 1) }}&text=Assalamualaikum Pak/Bu ,saya dapat informasi anda dari Binaan Halal ITS">Chat
                              Sekarang</a></h5>
                        </td>
                      </tr>
                      <tr>
...
@if(!empty($jsonld ?? ''))
@section('jsonld')
  <meta property="og:site_name" content="Pusat Kajian Halal ITS">
  <meta property="og:title" content="{{ $binaan->nama_umkm }}"/>
  <meta property="og:description" content="{{ $binaan->nama_produk }}"/>
  <meta property="og:image" itemprop="image"
        content="{{ $binaan->image ? asset('storage/products/' . $binaan->image) : asset('/ecommerce/img/logo_halal.png') }}">
  <script type="application/ld+json">
    @json($jsonld, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)
  </script>
@endsection
@endif
